...
public class WordCount implements ActionListener {
	...
	public void GUI(String[] args) {
		...
		BtnRun.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (Multifile.size() == 0) {
					JOptionPane.showMessageDialog(Frame, "Hãy chọn các file text");
					return;
				}
	...
	public static class MapForWordCount extends Mapper<LongWritable, Text, Text, Text> {
		public void map(LongWritable key, Text value, Context con) throws IOException, InterruptedException {
			...
			// lấy tên file
			String path = con.getInputSplit().toString();
			int offset = path.lastIndexOf("/") + 1;
			int count = path.lastIndexOf(".txt") - offset;
			String fileName = String.copyValueOf(path.toCharArray(), offset, count);
			// cắt các từ
			String[] words = line.split("[ ,.?!()\r\\n\"\\']");
			for (String word : words) {
				Text outputKey = new Text(word.toUpperCase().trim());
				if (keyText.indexOf(outputKey.toString()) != -1) {
	...
	public static class ReduceForWordCount extends Reducer<Text, Text, Text, Text> {
		public void reduce(Text word, Iterable<Text> values, Context con) throws IOException, InterruptedException {
			...
			textString += fileName + " " + sum + "}";
			con.write(word, new Text(textString));
		}
	}
